...
function assertCallOrder() {
    sinon.assert.callOrder.apply(sinon.assert, arguments);
    // callOrder doesn't verify last item
    sinon.assert.called(arguments[arguments.length-1]);
}
    ...
    describe("3.2.5.1: If/when `promise` is fulfilled, respective `onFulfilled` callbacks must execute in the order " +
        ...
        describe("`onFulfilled` handlers are called in the original order", function () {
            testFulfilled(dummy, function (promise, done) {
                ...
                promise.then(function () {
                    assertCallOrder(handler1, handler2, handler3);
                    done();
                });
            });
